...
### Summary
- Scanned: 0 new code commits since Run 44 (last commit `4dd8ed1` — nightshift docs only). Codebase unchanged since Run 17.
- Issues: 0 new, 0 resolved, 0 spoiler-leak P0. All 13 open planned issues confirmed unchanged (FIX-026, 027, 028, 029, 030, 045, 046, 047, 048, 049, 050, 051, 052). FIX-047 (12 stale model ID files) re-confirmed via grep. FIX-049 (requireKeith in 5 routes) re-confirmed via grep.
- Ideas (by theme): ask-forward — 1 seed (IDEA-126 — Ask Input Keyboard History), IDEA-123 promoted to planned; genmedia — 1 seed (IDEA-127 — Runway Valkyrie Exterior Cinematic), IDEA-115 parked (stale 3 days); post-read-world — 1 seed (IDEA-128 — World Faction Relations Matrix), IDEA-122 promoted to planned, IDEA-125 promoted to planned, IDEA-114 parked (stale 3 days). Total promoted: 3 (IDEA-122, IDEA-123, IDEA-125 → planned). Parked: 2 (IDEA-114, IDEA-115 — stale 3 days).
- Plans written: `DEVPLAN-IDEA-122-mission-day-coverage-chart.md`, `DEVPLAN-IDEA-123-ask-conversation-exporter.md`, `DEVPLAN-IDEA-125-completion-stamp.md`

### Build & Lint & Test Results
- No code commits since Run 44. Status confirmed unchanged from Run 36 verification.
- Build: **PASSES** (last verified Run 36 — no new code to re-validate)
- Lint: **PASSES** — 0 errors, 4 `<img>` tag warnings (unchanged since Run 17)
- Tests: **192 total / 192 PASS / 0 FAIL** (unchanged since Run 17)
...
1. **No new code commits.** Codebase identical to Run 44. All 13 open planned issues remain. The three fastest cleanups are unchanged: FIX-050 (5 min), FIX-049 (10 min), FIX-047 (15 min) = 30 minutes of zero-risk legacy cleanup. The queue now has 17 ready-or-planned ideas — IDEA-125 (10 min) and IDEA-123 (15 min) are the two fastest new entries tonight.

2. **Three ideas promoted to `planned` with dev plans written.** IDEA-123 (Ask Conversation Exporter, ask-forward): client-side Markdown download of current Ask session — `handleExport` callback + conditional button in `ask/page.tsx`, 15 min, 1-file, zero deps. IDEA-125 (Completion Stamp, post-read-world): CSS ink-stamp "MISSION COMPLETE · DIRECTIVE-14 · VALKYRIE-1" in the `/profile` header for `show_all_content` readers — 10 min, 1-file, ~5 lines JSX. IDEA-122 (Mission Day Coverage Chart, post-read-world): new `MissionDayCoverageChart.tsx` server component on `/stories/timeline` showing 21 mission days × chapter bar chart from `mission_logs_inventory.json` — reveals non-linear temporal architecture; 1 hour, 2 files.

3. **IDEA-122 architecture fully confirmed.** `TimelineView.tsx` is a server component (no `'use client'` — verified directly). `StoriesTimelinePage` is a simple one-liner wrapper. `mission_logs_inventory.json` parsed: 69 log entries across 21 unique mission days; non-linear structure confirmed (CH02 appears at Mission Days 38 AND 43, CH08 at Days 44 AND 113). The bar chart reveals this compression in one visual. Clean 2-file change.

4. **IDEA-114 and IDEA-115 parked (stale 3 days).** IDEA-114 (Ask Pre-Send Context Chips): exploring since Run 42, blocked on IDEA-096 shipping first. IDEA-115 (Entity Portrait Reveal in Ask): exploring since Run 42, hard-blocked by IDEA-052 (character portraits) prerequisite. Both parked per 3-day stale rule.

5. **Three new ideas seeded.** IDEA-126 (ask-forward: Ask Input Keyboard History — Up/Down arrow key history cycling in Ask textarea, ~20 lines, 15 min, zero deps). IDEA-127 (genmedia: Runway Valkyrie Exterior Cinematic — single 4-second ambient exterior clip via Runway Gen-4, ~$0.30 author-batch, ~15 min code, prerequisite FIX-048 + IDEA-106). IDEA-128 (post-read-world: World Faction Relations Matrix — static CH17 diplomatic status matrix on `/world/factions`, `show_all_content` gated, author-authored `FACTION_RELATIONS` constant, ~1 hour code + 15 min Paul authoring).

### Plans Ready to Execute

**New tonight:**
- `docs/nightshift/plans/DEVPLAN-IDEA-125-completion-stamp.md` — **NEW**: Completion Stamp on Profile — 10 min, 1-file, ~5 lines JSX (post-read-world). **Fastest ship in the entire queue.**
- `docs/nightshift/plans/DEVPLAN-IDEA-123-ask-conversation-exporter.md` — **NEW**: Ask Conversation Exporter — 15 min, 1-file, client-side Markdown download (ask-forward).
- `docs/nightshift/plans/DEVPLAN-IDEA-122-mission-day-coverage-chart.md` — **NEW**: Mission Day Coverage Chart — 1 hr, 2 files, `show_all_content` gated bar chart on timeline page (post-read-world).

**Previously queued (full list):**
- `docs/nightshift/plans/DEVPLAN-IDEA-120-ask-mood-responder.md` — Ask Mood Responder — 20 min, 4 files (ask-forward).
- `docs/nightshift/plans/DEVPLAN-IDEA-113-arc-progression-heatmap.md` — Arc Progression Heatmap — 2 new files, 2 hr (post-read-world).
- `docs/nightshift/plans/DEVPLAN-IDEA-116-world-crew-manifest.md` — World Crew Manifest — 1 new file, 1.5 hr (post-read-world).
...
### Recommendations
- **If you have 10 min:** IDEA-125 (Completion Stamp, 10 min) — the profile header gains a CSS ink stamp for completed readers. Single JSX block, zero risk. Fastest deliverable in the entire backlog.
- **If you have 30 min:** IDEA-123 (Conversation Exporter, 15 min) + FIX-050 (5 min) + FIX-049 (10 min) = two ships + two legacy cleanups. Or IDEA-120 (Ask Mood Responder, 20 min) for a fresh ask-forward feature with direct reader-facing impact.
- **If you have 1 hour:** IDEA-122 (Mission Day Coverage Chart, 1 hr) — the most structurally interesting post-read-world feature in the new queue; reveals how the novel's chapters map non-linearly onto mission days. Plus IDEA-125 in the same session (10 min).
- **If you have 2 hours:** IDEA-095 (Arc Endpoint Quotes Gallery, 1.5 hr) + IDEA-125 (10 min) + IDEA-123 (15 min) = the highest-leverage post-read-world foundation (`getArcEndpoints()` shared by IDEA-113, IDEA-116, IDEA-119) plus two fast ask-forward wins.
...
### Summary
- Scanned: 0 new code commits since Run 31 (last commit `e707805` — nightshift docs only). Codebase state unchanged.
- Issues: 0 new, 0 resolved, 0 spoiler-leak P0. All open planned issues unchanged: FIX-026, 027, 028, 029, 030, 045, 046, 047, 048, 049, 050, 051, 052. FIX-047 (12 stale model ID files) re-confirmed via grep; FIX-049 (requireKeith in 5 routes) re-confirmed; FIX-050 (`/\bnext\b/i` at `ask-intent.ts:35`) re-confirmed.
- Ideas (by theme): ask-forward 1 seed (IDEA-087 — Ask Source Deep-Dive Panel) / 1 promoted to `planned` (IDEA-084 — Ask Home Hero Widget, dev plan written); genmedia 1 seed (IDEA-088 — Approved Entity Thumbnail Inline) / 2 parked stale (IDEA-079 — Mission Briefing Art); post-read-world 1 seed (IDEA-089 — Completion Ceremony Page) / 1 parked stale (IDEA-080 — Personalized Reread Guide).
- Plans written: `DEVPLAN-IDEA-084-ask-home-hero-widget.md`

### Build & Lint & Test Results
- No code commits since Run 31. Build, lint, and test status unchanged: **PASSES** / 0 errors, 4 img warnings / 192 PASS.
- No build run performed (no code changes to validate; codebase identical to Run 31).

### Key Findings

1. **No new code commits.** Codebase is identical to Run 31. All prior open issues remain in documented states. FIX-047 (12 stale model ID files — confirmed via grep returning 12 source files: `personas.ts`, `synthesize-prompt.ts`, `extract-vision.ts`, `session-wrap.ts`, `profile-reflection.ts`, `ledger.ts`, `tell/route.ts`, `beyond/polish/route.ts`, `tell/draft/route.ts`, plus 3 test files), FIX-049 (requireKeith in 5 visuals routes — confirmed via grep), FIX-050 (overly broad `/\bnext\b/i` at `ask-intent.ts:35` — confirmed via grep) all still open.

2. **Review queue: 1 entry with `reviewed: false`.** `brain_lab/out/review-queue.md` is 15 lines total, 1 entry with `reviewed: false`. File timestamp unchanged from Run 31 — pipeline not re-run.

3. **IDEA-084 promoted to `planned` and dev plan written.** Ask Home Hero Widget — adds a text input form to `HomeHero.tsx` (frosted-glass input + "Ask →" button, ~35 lines) and a new `?q=` auto-submit effect in `ask/page.tsx` (~8 lines). Reader types a question on the home page, navigates to `/ask?q=...`, and the question auto-submits on mount. 2-file change. The `?q=` auto-submit pattern mirrors the existing `?passage=` pattern (lines 553–575 in `ask/page.tsx`). Estimated 45 minutes. Plan: `DEVPLAN-IDEA-084-ask-home-hero-widget.md`.

4. **Two ideas parked (3-day stale rule).** IDEA-079 (genmedia: Mission Briefing Classified Document Art — seeded 2026-05-14, 3 days old; blocked by missing faction spec JSON) and IDEA-080 (post-read-world: Personalized Reread Guide — seeded 2026-05-14, 3 days old; superseded in part by IDEA-086 Reading Journey Timeline). Both demoted to parked.

5. **Three new ideas seeded.** IDEA-087 (ask-forward: Ask Source Deep-Dive Panel — expandable entity detail accordion on each `AskSourcesDisclosure` source link, showing type badge + description + wiki link; ~2 hours; synergistic with IDEA-063 EntityHoverCard), IDEA-088 (genmedia: Approved Entity Thumbnail Inline — small circular portrait thumbnails next to wiki entity links in Ask answers, using only pre-approved `cel_visual_assets`; zero generation cost for readers; ~2 hours after IDEA-052 ships; supersedes parked IDEA-070), IDEA-089 (post-read-world: Completion Ceremony Page — celebration `/completion` page for `show_all_content` readers showing reading stats, highlight fingerprint, and post-read-world feature discovery; ~2 hours baseline; plug-in for IDEA-077 mosaic after that ships).

6. **Priority queue status.** 11 ideas are `ready` or `planned` with written dev plans. IDEA-084 (45 min — Ask Home Hero Widget) is the new quickest win. IDEA-048 (15 min — Ask CTA near top of story page) remains fastest overall.

### Plans Ready to Execute
- `docs/nightshift/plans/DEVPLAN-IDEA-084-ask-home-hero-widget.md` — **NEW**: Ask Home Hero Widget — text input in home page hero, routes to `/ask?q=...` with auto-submit; 2-file change, 45 min (ask-forward).
- `docs/nightshift/plans/DEVPLAN-IDEA-078-ask-confidence-ring.md` — Ask Response Confidence Ring — left-border grounding signal on answer bubbles; 1-file JSX change, 20 min (ask-forward).
- `docs/nightshift/plans/DEVPLAN-IDEA-048-ask-cta-top-of-story-page.md` — Ask CTA after chapter summary (ask-forward). 15 minutes.
- `docs/nightshift/plans/DEVPLAN-IDEA-057-context-aware-ask-welcome.md` — Context-aware Ask welcome + chapter chips (ask-forward). 45 minutes.
- `docs/nightshift/plans/DEVPLAN-IDEA-051-scene-level-ask-affordance.md` — Scene-level "Ask →" hover affordance (ask-forward). 30 minutes.
- `docs/nightshift/plans/DEVPLAN-IDEA-063-entity-hover-card.md` — Entity hover-card in Ask answers (ask-forward). 30 minutes.
- `docs/nightshift/plans/DEVPLAN-IDEA-075-ask-pinned-qa.md` — Ask Pinned Q&A — star-and-save exchanges, migration 042 required (ask-forward). 2.5 hours.
- `docs/nightshift/plans/DEVPLAN-IDEA-077-highlight-fingerprint.md` — Highlight Fingerprint mosaic on `/profile/highlights`, gated by `show_all_content` (post-read-world). 1.5 hours.
- `docs/nightshift/plans/DEVPLAN-IDEA-062-re-reader-hindsight-panel.md` — Re-Reader Hindsight Panel (post-read-world). 2 hours.
- `docs/nightshift/plans/DEVPLAN-IDEA-042-follow-up-chips.md` — Suggested follow-up chips after Ask answers (ask-forward). 2 hours.
- `docs/nightshift/plans/FIXPLAN-FIX-050-ask-intent-next-pattern.md` — Remove `/\bnext\b/i` from FUTURE_PATTERNS (5 min).
...
### Recommendations
- **If you have 30 min:** IDEA-048 (15 min — Ask CTA after story summary) + FIX-050 (5 min — remove over-broad next pattern) + FIX-049 (10 min — rename requireKeith). Three quick wins, zero dependency risk.
- **If you have 1 hour:** The 30-min cluster above + IDEA-084 (45 min — Ask Home Hero Widget) — makes Ask the primary entry action from the home page in two clean files.
- **If you have 2+ hours:** IDEA-062 (2 hr — Re-Reader Hindsight Panel) + IDEA-077 (1.5 hr — Highlight Fingerprint). Two complementary post-read-world features that each stand alone but share the `show_all_content` gate pattern.

---

## Run: 2026-05-16 (Run 31)

### Summary
- Scanned: 0 new code commits since Run 30 (last commit `fc1dc55` — nightshift docs only). Codebase state unchanged.
...
- Ideas (by theme): ask-forward 1 seed (IDEA-084 — Ask Home Hero Widget) / 1 promoted to `planned` (IDEA-078 — Confidence Ring, dev plan written); genmedia 1 seed (IDEA-085 — Ask Ink Print) / 1 parked (IDEA-076 — World Visual Glossary, 3-day stale); post-read-world 1 seed (IDEA-086 — Reading Journey Timeline).
- Plans written: `DEVPLAN-IDEA-078-ask-confidence-ring.md`

### Build & Lint & Test Results
- No code commits since Run 30. Build, lint, and test status unchanged: **PASSES** / 0 errors, 4 img warnings / 192 PASS.
- No build run performed (no code changes to validate; codebase identical to Run 30).

### Key Findings

1. **No new code commits.** Codebase is identical to Run 30. All prior open issues remain in documented states. FIX-047 (12 stale model ID files — confirmed via grep), FIX-049 (requireKeith in 5 visuals routes — confirmed via grep), FIX-050 (overly broad next pattern at `ask-intent.ts:35` — confirmed via grep) all still open.

2. **Review queue count: 1 entry with `reviewed: false`.** `brain_lab/out/review-queue.md` file timestamp still 2026-04-26 — pipeline not re-run. Count consistent with Runs 23–30; accurate count requires a fresh `brain_lab` pipeline run.

3. **IDEA-078 promoted to `planned` and dev plan written.** Ask Response Confidence Ring — adds a subtle left-border accent to assistant message bubbles in the Ask companion derived from `linksInAnswer.length` already in client state. Ocean accent (≥3 links), muted clay accent (1–2 links), no accent (0 links). Single file change: `src/app/ask/page.tsx` lines 698–703 (bubble `<div>` className). Zero API changes, zero DB, zero new imports. Estimated 20 minutes. Plan: `DEVPLAN-IDEA-078-ask-confidence-ring.md`. Caveat noted in plan: `--color-ocean` CSS var availability should be verified in `globals.css` before executing.

4. **IDEA-076 parked (3-day stale rule).** World Visual Glossary seeded 2026-05-13 (3 days ago). Idea is sound but requires `cel_visual_assets` to be populated first (IDEA-052 character portraits must ship). Moved to Parked with note.

5. **Three new ideas seeded.** IDEA-084 (ask-forward: Ask Home Hero Widget — a text input in the home page hero that routes to `/ask?q={encodedQ}`, making Ask the primary home page action; extends `ask/page.tsx` to read `?q=` param and auto-submit; ~45 min), IDEA-085 (genmedia: Ask Ink Print — author curates a favorite Ask exchange from the AI ledger and generates a poster-style typographic Imagen 4 image; displayed on an `/about/ink-print` or featured-exchange page; zero reader cost; dual-use as Ask onboarding showcase), IDEA-086 (post-read-world: Reading Journey Timeline — personal `/profile/journey` page for `show_all_content` readers showing the date each chapter was first read + highlights count + Ask question count per chapter; three existing Supabase queries; ~2 hours).

6. **Open priority queue is deep.** 10 ideas are at `ready` status with written dev plans — the queue is saturated. No new items need to be promoted this run beyond IDEA-078. Key quick-wins still at the top: IDEA-048 (15 min — Ask CTA after story summary), IDEA-078 (20 min — Confidence Ring), FIX-050 (5 min — remove overly broad next pattern), FIX-049 (10 min — rename requireKeith).

### Plans Ready to Execute
- `docs/nightshift/plans/DEVPLAN-IDEA-078-ask-confidence-ring.md` — **NEW**: Ask Response Confidence Ring — left-border grounding signal on answer bubbles; 1-file JSX change, 20 min (ask-forward).
- `docs/nightshift/plans/DEVPLAN-IDEA-075-ask-pinned-qa.md` — Ask Pinned Q&A — star-and-save exchanges to `/profile/questions`, migration 042 required (ask-forward). 2.5 hours.
- `docs/nightshift/plans/DEVPLAN-IDEA-077-highlight-fingerprint.md` — Highlight Fingerprint mosaic on `/profile/highlights`, gated by `show_all_content` (post-read-world). 1.5 hours.
- `docs/nightshift/plans/DEVPLAN-IDEA-048-ask-cta-top-of-story-page.md` — Ask CTA after chapter summary (ask-forward). 15 minutes.
- `docs/nightshift/plans/DEVPLAN-IDEA-057-context-aware-ask-welcome.md` — Context-aware Ask welcome + chapter chips (ask-forward). 45 minutes.
- `docs/nightshift/plans/DEVPLAN-IDEA-051-scene-level-ask-affordance.md` — Scene-level "Ask →" hover affordance (ask-forward). 30 minutes.
...
### Recommendations
- **If you have 30 min:** IDEA-048 (15 min — Ask CTA after story summary) + FIX-050 (5 min — remove over-broad next pattern) + FIX-049 (10 min — rename requireKeith). Three quick wins, zero dependency risk.
- **If you have 1 hour:** The 30-min cluster above + IDEA-078 (20 min — Confidence Ring, 1-file JSX change) + IDEA-063 (30 min — entity hover-card in Ask answers). Five ask-forward improvements, all pure client-side or small component additions.
...
### Summary
- Scanned: 0 new code commits since Run 22 nightshift (`c5e3a1f` was docs-only). Codebase state unchanged.
- Issues: 0 new. 0 resolved. 0 spoiler-leak P0. All open planned issues unchanged: FIX-026, 027, 028, 029, 030, 045, 046, 047, 048, 049, 050, 051.
- Ideas (by theme): ask-forward 1 seed (IDEA-060 — conversation history browser) / 1 promoted to `planned` (IDEA-057 — context-aware welcome message, dev plan written); genmedia 1 seed (IDEA-061 — chapter completion atmospheric video loop) / 0 promoted; post-read-world 1 seed (IDEA-062 — re-reader hindsight panel) / 0 promoted; parked 1 (IDEA-053 — Valkyrie-1 interior map, 3-day stale rule).
- Plans written: `DEVPLAN-IDEA-057-context-aware-ask-welcome.md`.

### Build & Lint & Test Results
- No code commits since Run 22. Build, lint, and test status unchanged: **PASSES** / 0 errors, 4 img warnings / 192 PASS.
- No build run performed this session (no code changes to validate; codebase identical to Run 22).

### Key Findings

1. **No new code commits.** Codebase is identical to Run 22. All prior open issues and parked issues remain in last-known state. FIX-049 and FIX-050 confirmed still open via direct grep. Review queue still shows 1 entry with `reviewed: false` (same as Run 21 count — pipeline not re-run).

2. **IDEA-057 promoted to `planned` and dev plan written.** Context-aware welcome message on the Ask page when `?story={storyId}` is present. Key implementation insight: `ask/page.tsx` is a `'use client'` file, so server-only `getChapterTags()` (which uses Node `fs`) cannot be called directly. The cleanest injection point is the existing `/api/stories/[storyId]/meta` GET endpoint that the Ask page already fetches for `contextStoryTitle`. Extend the route to also return `chapterWelcome: { greeting, suggestions }` computed from `getChapterTags(storyId)` on the server. The Ask client adds a second state variable alongside `contextStoryTitle` and branches the empty-state render. Zero extra API calls. Estimated 45 minutes. Two-file change: `meta/route.ts` + `ask/page.tsx`.

3. **IDEA-053 parked (3-day stale rule).** Valkyrie-1 Interactive Interior Map was seeded 2026-05-04 — exactly 3 days ago. Blocker is SVG authoring (design-time dependency); no code path available without Paul committing to layout authorship. Demoted to parked.

4. **Three new ideas seeded.** IDEA-060 (ask-forward: Ask conversation history browser — `/ask/history` page grouping past `cel_conversations` by chapter with keyword search; zero new data model), IDEA-061 (genmedia: chapter completion video loop — short Runway Gen-4 clips pre-generated by author, triggered on chapter-complete event, $1.02 total for 17 chapters), IDEA-062 (post-read-world: re-reader hindsight panel — collapsible "Hindsight" accordion on chapter pages gated by `show_all_content`, sourced from arc ledger milestone notes, zero new content needed).

5. **Stale model IDs (FIX-047) and `requireKeith` naming (FIX-049) confirmed unchanged.** Both remain quick wins (<15 min each) at the top of the priority queue.

6. **`ask/page.tsx` age mode surface confirmed.** The Ask page has 7 references to `ageMode` (imports `useAgeMode`, sends `ageMode` in API payload at line 406, `SUGGESTIONS_BY_AGE_MODE` on line 679). FIX-029 plan already covers these. Noting for Paul: Phase 1 of FIX-029 must also remove the `ageMode` from the Ask API request body (line 406) and clear the `readStoredAskMode()` localStorage call — not just the UI switcher. The existing FIX-029 plan covers this.

7. **Review queue hygiene.** `brain_lab/out/review-queue.md` grep confirms 1 entry with `reviewed: false` (consistent with Run 21's count; still down from 9 in prior runs). No pipeline re-run; count may be stale. Timestamp on file is still 2026-04-26 — a fresh `brain_lab` run would give an accurate count.

### Plans Ready to Execute
- `docs/nightshift/plans/DEVPLAN-IDEA-057-context-aware-ask-welcome.md` — **NEW**: Chapter-grounded welcome message + 3 entity-based suggestion chips in Ask empty state (ask-forward). 45 minutes.
- `docs/nightshift/plans/DEVPLAN-IDEA-051-scene-level-ask-affordance.md` — Scene-level "Ask →" hover on `### Scene` headings (ask-forward). 30 minutes.
- `docs/nightshift/plans/DEVPLAN-IDEA-048-ask-cta-top-of-story-page.md` — Ask CTA after chapter summary (ask-forward). 15 minutes.
- `docs/nightshift/plans/FIXPLAN-FIX-051-dangerouslysetinnerhtml-admin.md` — HTML sanitization for admin dangerouslySetInnerHTML. 1 hour.
- `docs/nightshift/plans/FIXPLAN-FIX-050-ask-intent-next-pattern.md` — Remove overly broad `/\bnext\b/` from ask-intent. 5 minutes.
...
### Recommendations
- **If you have 30 min:** IDEA-057 (45 min — context-aware Ask welcome, chapter-specific chips) OR the pair of IDEA-048 (15 min) + FIX-050 (5 min) + FIX-049 (10 min). Ask-forward quick win cluster.
- **If you have 1 hour:** IDEA-048 + IDEA-051 + IDEA-057 (15 + 30 + 45 = 90 min, or 60 if you skip IDEA-057). All three are pure JSX/API additions — no new imports, no DB changes, no dependency risk.
- **If you have 2 hours:** The 1-hour cluster above + FIX-051 (HTML sanitization setup: npm install + create sanitize-html.ts + wrap both call sites). After this session: three ask-forward improvements live and the one security-adjacent fix done.
