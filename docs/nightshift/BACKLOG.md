...
> Last updated: 2026-06-02 (Run 48)
...
- **Night Notes:**
  ...
  - 2026-05-27 (Run 42): **Promoted to `ready`.** Dev plan confirmed present and self-contained. No new blockers. Ready to execute — smallest toggle in the queue.
...
- **Night Notes:**
  ...
  - 2026-05-27 (Run 42): **Promoted to `ready`.** Dev plan confirmed present. No new blockers. Ready to execute — brings passive Ask discovery to every chapter page with zero API overhead.
...
- **Night Notes:**
  ...
  - 2026-05-27 (Run 42): **Promoted to `ready`.** Dev plan confirmed present and self-contained. No new blockers. The 17-chapter discovery grid is the fastest way to give cold Ask visitors an immediate entry point.
...
- **Night Notes:**
  ...
  - 2026-05-27 (Run 42): **Promoted to `ready`.** Dev plan confirmed present. Fastest win in the Ask navigation queue — 30 min, 1 file, zero API changes. Makes every grounded Ask answer directly navigable back to the source chapter.
...
- **Night Notes:**
  ...
  - 2026-06-02 (Run 48): **Promoted to `ready`.** Dev plan confirmed present and self-contained: `DEVPLAN-IDEA-126-ask-input-keyboard-history.md`. 15 min, 1-file change. Zero API, zero DB.
...
- **Last Updated:** 2026-06-02
- **Priority:** P3
- **Plan:** `docs/nightshift/plans/DEVPLAN-IDEA-132-ask-session-summary-card.md`
...
- **Night Notes:**
  ...
  - 2026-06-02 (Run 48): **Promoted to `planned`.** Dev plan written: `DEVPLAN-IDEA-132-ask-session-summary-card.md`. 2-file change: `ask/page.tsx` (~35 lines: idle timer refs + useEffect + Message type extension + summary card render) + new `src/app/api/ask/summary/route.ts` (~30 lines: POST route calling Haiku). Priority set to P3. Estimated 30 minutes.
...
- **Night Notes:**
  ...
  - 2026-06-02 (Run 48): **Promoted to `ready`.** Dev plan confirmed present and complete: `DEVPLAN-IDEA-129-ask-greeting-personalization.md`. No new blockers.

---

### [IDEA-135] Ask Answer Fact-Check Button — On-Demand Citation Footnote per Response
- **Status:** seed
- **Theme:** ask-forward
- **Seeded:** 2026-06-02
- **Last Updated:** 2026-06-02
- **Priority:** unranked
- **Plan:** *(not yet written)*
- **Summary:** A small "✓ Fact-check" button below each Ask assistant response bubble (visible on hover). Clicking triggers a secondary Haiku call that lists the 3 most specifically relevant wiki entries to the answer with a one-line explanation each — appended as a compact citations footnote to that bubble. Distinct from `AskSourcesDisclosure` (which shows linked entities) in that it produces an explicit ranked relevance explanation. On-demand, ~$0.001/click.
- **Night Notes:**
  - 2026-06-02 (Run 48): Seeded. `AskSourcesDisclosure` already shows entity links extracted from the answer, but does not explain WHY those entities are relevant or rank them. The fact-check button adds an on-demand deep-citation layer: (1) A `<button className="...opacity-0 group-hover:opacity-100">✓ Fact-check</button>` on each assistant bubble; (2) Clicking POSTs to a new `/api/ask/factcheck` route with `{ question: string, answer: string, entityLinks: string[] }` — calls `claude-haiku-4-5-20251001` with "Given this question and answer, which 3 wiki entries from the provided entity links are most directly relevant? For each: entity name + one-line explanation."; (3) Response appended as a small `<ul>` footnote below the bubble in muted text. Idempotent — result cached in component state keyed by `message.id`. (1) Model/provider: `claude-haiku-4-5-20251001`. (2) Cost: ~$0.001/click — lazy/on-demand only. (3) Caching: component-state keyed by message ID — no DB. (4) Spoiler gating: entity links are already visible to the reader in `AskSourcesDisclosure` — no new content surfaced. (5) Canon grounding: draws from `entityLinks` already returned in `done` SSE event — no new wiki fetches. Estimated 30–45 minutes: new `/api/ask/factcheck` route + hover button + footnote render in `ask/page.tsx`. Synergistic with IDEA-087 (Source Deep-Dive) and IDEA-078 (Confidence Ring) — all three add depth to the post-answer bubble area.
...
- **Night Notes:**
  ...
  - 2026-05-27 (Run 42): **Promoted to `ready`.** Dev plan confirmed present and complete. FIX-048 remains the prerequisite; plan is otherwise self-contained. The Valkyrie-1 wiki page gaining a dynamic harmonic state header is the most cinematic genmedia win before IDEA-043 ships.
...
- **Night Notes:**
  ...
  - 2026-05-27 (Run 42): Stale 3 days — likely low priority or too complex. Demoting to parked. Pure author-batch task with zero code changes; un-park when Paul is running a vault asset generation session via the admin console.
...
- **Night Notes:**
  ...
  - 2026-05-27 (Run 42): Stale 3 days — likely low priority or too complex. Demoting to parked. Blocked on Paul's authoring of the 17 mood→color mapping pairs before code work can begin. Un-park when that mapping is ready.
...
- **Night Notes:**
  ...
  - 2026-06-02 (Run 48): **Promoted to `ready`.** Dev plan confirmed present and complete. Prerequisite (FIX-048 + IDEA-106) remains soft — page renders without video if not yet generated. Ready for author batch session + 15-min code addition.
...
- **Night Notes:**
  ...
  - 2026-06-02 (Run 48): Remaining in `exploring`. The soft prerequisite (IDEA-052 shipping to prove the admin console batch workflow) has not yet been met. When IDEA-052 ships, advance directly to `planned` — the technical design is fully specified from the exploring analysis. No new blockers identified.
...
### [IDEA-136] Character "First Appearance" Portrait — Chapter-Grounded Debut Portrait per Crew Member
- **Status:** seed
- **Theme:** genmedia
- **Seeded:** 2026-06-02
- **Last Updated:** 2026-06-02
- **Priority:** unranked
- **Plan:** *(not yet written)*
- **Summary:** For each of the 9 main crew members, author pre-generates a single Imagen 4 portrait specifically grounded in the chapter text describing their **first appearance** — what they looked like, wore, and projected when introduced to the reader. Style: `intimate_crew` preset; prompt includes the character's first-chapter entity context from `chapter_tags.json` + the character wiki's intro paragraph. Distinct from IDEA-052 (canonical headshot, timeless) — this captures the character's debut moment specifically. Author-batch only; ~$0.54 total. Zero code changes if portraits are approved to existing `EntityVisualsGallery` slot with `style='first_appearance'`.
- **Night Notes:**
  - 2026-06-02 (Run 48): Seeded. IDEA-052 generates a canonical "best portrait" per character. This idea generates a debut-specific portrait — the reader's first impression of each character: what they were wearing, their posture, the setting. This is a richer reading companion artifact than a general headshot because it's moment-specific. (1) Model/provider: Imagen 4 with `intimate_crew` preset for crew members; `noncorporeal_presence` for ALARA (whose first appearance is non-visual/ambient). ~$0.06/image × 9 = $0.54. (2) Cost: author-batch only, one-time, ~$0.54 total. (3) Caching: stored in `cel_visual_assets` with `target={characterSlug}`, `style='first_appearance'`, `approved=true`; shared canonical. Surfaced by existing `EntityVisualsGallery` on character detail pages automatically. (4) Spoiler gating of prompt inputs: prompt uses only the character's FIRST chapter context — no arc-endpoint data, no later-chapter information. The prompt is constructed from: `chapter_tags.json` entity description for the character's first-tagged chapter + the intro paragraph of the character wiki markdown (`content/wiki/characters/{slug}.md`). Safe for all readers since this depicts the earliest available information. (5) Canon grounding: character wiki intro paragraph + `chapter_tags.json` first-chapter entity description + `intimate_crew` preset for human crew, `noncorporeal_presence` for ALARA. Prerequisite: character wiki markdown already exists for all 9 characters; no spec JSON required (text-only grounding). Author workflow: extract the first-tagged chapter per character from `chapter_tags.json`, paste the relevant paragraph from the character wiki as prompt context, generate via admin console, approve. Estimated: 30 minutes author time in admin console (batch 9 portraits). Zero code changes — `EntityVisualsGallery` already renders multiple `style`-keyed assets per character. Synergistic with IDEA-052 (general canonical headshot) — a character page could show both "First Appearance" and "Canonical" portraits as two distinct gallery slots, creating a visual before/after arc story.
...
- **Night Notes:**
  ...
  - 2026-05-27 (Run 42): Stale 3 days — likely low priority or too complex. Demoting to parked. The `/rules` index page already surfaces all 25 rules; the glossary adds grouping and keyword filter convenience. Un-park when the `/world/` cluster is being actively developed alongside IDEA-095 and IDEA-104.
...
- **Night Notes:**
  ...
  - 2026-05-27 (Run 42): Stale 3 days — likely low priority or too complex. Demoting to parked. The location-chapter grid is compelling but requires IDEA-095 to ship first to establish the `/world/` nav cluster. Un-park when IDEA-095 lands and the `/world/` pages are being actively built.
...
- **Last Updated:** 2026-05-27
- **Priority:** P2
- **Plan:** `docs/nightshift/plans/DEVPLAN-IDEA-116-world-crew-manifest.md`
...
- **Night Notes:**
  ...
  - 2026-05-27 (Run 42): **Promoted to `planned`.** Dev plan written: `DEVPLAN-IDEA-116-world-crew-manifest.md`. Phase 1 buildable today: crew names + roles/species from character wiki files + "MARU CLASSIFIED" status placeholder. Phase 2 (after IDEA-095 ships): swap placeholder for `getArcEndpoints()` real data — 2-line change. Priority raised to P2. Estimated 1.5 hours (including reading 9 character wiki files to populate `CREW_META`). 1 new file: `src/app/world/manifest/page.tsx`.
...
- **Night Notes:**
  ...
  - 2026-06-02 (Run 48): **Promoted to `ready`.** Dev plan confirmed present and self-contained. 10 min, 1-file change. Fastest post-read-world ship in the queue.
...
- **Night Notes:**
  ...
  - 2026-06-02 (Run 48): **Promoted to `ready`.** Dev plan confirmed present and complete. 1 new file, ~80 lines, ~1 hr code + Paul authors the `FACTION_RELATIONS` constant (15 min). No blockers.
...
- **Night Notes:**
  ...
  - 2026-06-02 (Run 48): Remaining in `exploring`. Soft prerequisite (IDEA-116 shipping) still pending. When IDEA-116 lands, advance to `planned` — the full dev plan can be derived from IDEA-116's implementation with a different data layout. No new blockers.
...
- **Night Notes:**
  ...
  - 2026-06-02 (Run 48): Remaining in `seed`. The remark-plugin phrase-matching approach is the right path but adds ~3 hours of complexity. Advance to `exploring` when IDEA-062 (Hindsight Panel) ships and the arc-parsing utilities are confirmed working. The annotation JSON content schema is well-defined — Paul can author CH01 sample annotations at any time.

---

### [IDEA-137] Hidden Canon Tour — Author-Curated Foreshadowing Callback Pairs for Completed Readers
- **Status:** seed
- **Theme:** post-read-world
- **Seeded:** 2026-06-02
- **Last Updated:** 2026-06-02
- **Priority:** unranked
- **Plan:** *(not yet written)*
- **Summary:** For `show_all_content=true` readers, a `/world/tour` page presenting 5–7 author-curated "Canon Callbacks" — specific moments where an early chapter detail pays off in a late chapter. Each callback shows: an early-chapter quote → what it foreshadowed → the payoff-chapter quote, with "↗ Ask about this" links to `/ask?story={ch}` for both chapters. Paul authors the callbacks in `content/wiki/tour/callbacks.md` as structured markdown sections. A "director's commentary" layer: Paul surfaces the intentional craft behind the story. Zero AI generation, zero DB, zero npm.
- **Night Notes:**
  - 2026-06-02 (Run 48): Seeded. Completed readers often want to understand the craft behind the narrative — what was planted in Chapter 2 that paid off in Chapter 15. The `/world/tour` page transforms this into an author-guided tour through deliberate callbacks. Implementation: (1) Paul authors `content/wiki/tour/callbacks.md` — no `<!-- generated:ingest -->` marker (manually owned). Schema per callback: `## Callback N: [title]`, `**Planted:** [quote] (CH0X, Scene Y)`, `**Payoff:** [quote] (CH0Z, Scene W)`, `**Commentary:** [1-2 author sentences]`. Markdown structure makes it easily parseable. (2) New `src/app/world/tour/page.tsx` — server component, `show_all_content` gate via `getReaderProgress()` (redirect to `/` if false); reads `content/wiki/tour/callbacks.md` via `fs.readFile`; parses into sections by `## Callback N:` headings using a simple split; renders each callback as a two-panel card (early → payoff) with `react-markdown` for the quote text + "Ask about CH0X →" link below each panel. (3) Post-read-world requirements: (a) Page hidden from first-time/guest readers — server `show_all_content` gate; (b) `show_all_content` integration: direct server-side check; (c) Partial-completion: flag is sole gate. Zero new DB, zero AI, zero npm. Estimated: 1 hour code + Paul writes 5–7 callback entries (30 min). Synergistic with IDEA-095 (`/world/voices`) and IDEA-128 (`/world/factions`) — adds a third post-read `/world/` archive page with a distinct editorial character. Add a nav link from `/world/voices` to `/world/tour`. Prerequisite: none — the markdown authoring workflow is independent of any other shipped feature.
